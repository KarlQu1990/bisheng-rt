...
class BaseEmbedding(object):
    ...
    def _load(
        ...
        if jina_mode is True:
            self.model = JinaBertModel.from_pretrained(
                pretrain_path, device_map=device_map, trust_remote_code=True, use_safetensors=use_safetensors
            )
        else:
...
class BGEM3EmbResponse(EmbResponse):
